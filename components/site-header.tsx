...
export function SiteHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-wine/10 bg-ivory/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="hidden items-center gap-2 md:flex">
          <Button
            variant="ghost"
            size="lg"
            className="text-ink hover:bg-wine/5 hover:text-wine"
            nativeButton={false}
      ...
      {open && (
        <div className="border-t border-wine/10 bg-ivory md:hidden">
          <nav className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-4" aria-label="Navigation mobile">
            {navLinks.map((link) => {
              return (
                <a
                  ...
                  className="rounded-md px-2 py-2 text-sm font-medium text-ink/70 transition-colors hover:bg-wine/5 hover:text-wine"
                >
                  {link.label}
                </a>
              )
            ...
            <div className="mt-3 flex flex-col gap-2">
              <Button
                variant="outline"
                size="lg"
                className="border-wine/20 text-ink"
                nativeButton={false}
                render={<a href="/connexion" />}
              >
                Se connecter
